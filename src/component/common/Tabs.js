...
const Tabs = props => {
  ...
  const [isFromDL, setIsFromDL] = useState(false);
  ...
  useEffect(() => {
    if (props.route && props.route.params && props.route.params.parentTab) {
      ...
      if (
        props?.navigation?.getState()?.routes?.length &&
        props?.navigation?.getState()?.routes?.length - 1 > 1 &&
        ...
        if (route?.params?.childTab == 'EditAddress') {
          props.navigation.navigate('EditAddress');
          setIsFromDL(true);
        } else if (route?.params?.childTab == 'Pickup') {
          onItemPress(1);
          setIsFromDL(true);
        }
      }
